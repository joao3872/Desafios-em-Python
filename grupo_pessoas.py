# Confere quantas mulheres mais novas tem no grupo, e mostra o homem mais velho, mais algumas opções.
...
maior20 = 0 
for c in range(1, 5):
    nome = input(f'Informe o nome da {c}° pessoa: ').strip().upper()
    idade = int(input(f'Informe a idade da {c}° pessoa: '))
    sexo = input(f'Informe o sexo da {c}° pessoa com (M / F): ').strip().upper()
    print(' ')
    media += idade / 4
    if(sexo == 'M'):
        homem += 1
